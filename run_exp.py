...
#         rss = mem_info.rss / (1024 ** 2)      # 单位 MB
#         vms = mem_info.vms / (1024 ** 2)
#         print(f"[{time.strftime('%H:%M:%S')}] RSS={rss:.2f}MB  VMS={vms:.2f}MB")
#         time.sleep(interval)
...
if __name__ == "__main__":
    ...
    # input()
    # import threading
    # t = threading.Thread(target=monitor, args=(2,), daemon=True)
    # t.start()
    ...
    parser.add_argument(
        ...
        type=str,
        default="LPNeSy",
        choices=["RuleNeSy", "LLMNeSy", "ReAct", "Act", "LPNeSy"],
    )
    parser.add_argument(
        "--llm",
        "-l",
    ...
    for i, data_idx in enumerate(query_index):
        # data_idx = "e20241028160307743716"
